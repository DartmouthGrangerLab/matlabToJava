...
//		}
		...
		for (int k = 0; k < vts.size(); k++) {
			...
			// XXX: double to int array shifting is wasteful. Consider changing Segments to accept double array or even a DenseMatrix
		...
		int minDiffIdx = Collections.indexOfSubList(diff, search);
		...
		if ((minDiff * imageXSums.numRows) > VT_MATCH_THRESHOLD) {
			vts.add(new VT(numvts, imageXSums.data, VT_ACTIVE_DECAY, x_val, y_val, th_val, 1, 0));
			vt_id = numvts;
		} else {
			vt_id = minDiffIdx;
